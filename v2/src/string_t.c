...
{
	string_assign_from_array(s, arr, length);
}

void string_concatenate(string_t *a, string_t *b)
{
	string_size_at_least(a, a->elements + b->elements);
...
{
	string_size_at_least(s, s->elements + n);
	...
	*num = 0;
	...
	for (i = 0; i < s->elements; i++)
	{
		if (s->array[i] == delim)
		{
			...
			ret_val[*num - 1] = string_initialize();
...
{
	string_size_at_least(s, s->elements + 1);
...
{
	while (string_get(s, 0) == ' ')
	{
		string_remove(s, 0);
	}

	size_t last = string_length(s);
	while (string_get(s, last - 1) == ' ')
	{
		string_pop_back(s);
